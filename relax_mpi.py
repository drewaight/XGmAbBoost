...
def relax(path, loc_cmplx, rank, repeat):
  start_time = timeit.default_timer()
  with concurrent.futures.ThreadPoolExecutor() as executor:
    future = executor.submit(drew_relax.main, path, loc_cmplx, repeat)
    sc = future.result()
...
def main(listfile, path, repeat):
  ...
  for z in z_scatt:
    ...
    zin = int(zi)
    loc_cmplx = list[zin]
    sc = relax(path, loc_cmplx, rank, repeat)
  ...
  if rank == 0:
    ...
    df.to_json(os.path.join(path, 'relaxed_list.json'))
